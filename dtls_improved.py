...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.discriminator = d_3(image_size=self.image_size, dim=32,
                                 dim_mults=(4, 2, 2, 1, 1),channels=3).to(device)
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            # loss_sim = self.lpips(x_recon, x_start).mean()
            ...
            loss_gen = self.BCE_loss(score_fake, GAN_fake) * 1e-3

            (loss + loss_gen).backward()
            # loss.backward()
            self.opt.step()
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                LQ_maniqa, LQ_niqe = self.evaluation()

                if LQ_maniqa > self.best_quality:
                    self.best_quality = LQ_maniqa
                    self.save_best()
                self.metrics_list.append(f"{self.step} LQ Images MANIQA: {LQ_maniqa} | NIQE: {LQ_niqe}    "
                                         f"FFHQ Images MANIQA: {FFHQ_quality_MANIQA} | NIQE: {FFHQ_quality_NIQE}")
    ...
    def evaluation(self):
        ...
        for idx, path in enumerate(sorted(glob.glob(os.path.join(self.input_image, '*')))):
            ...
            utils.save_image(img_sr, self.results_folder /f'{imgname}.png', nrow=self.nrow)
        ...
        img_set = torch.cat((blur_img_set, hq_img_set), dim=0)
        utils.save_image(img_set, str(self.results_folder / f'overall/{self.step}_overall.png'), nrow=blur_img_set.shape[0])
