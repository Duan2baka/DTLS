...
parser.add_argument('--stride', default=16, type=int, help="size change between each step if linear mode is used")
...
parser.add_argument('--save_folder', default="GDTLS_16_128", type=str, help="Folder to save your train or evaluation result")
...
if args.hr_size == 1024:
    ...
    discriminator = discriminator_v3(
        ...
        dim_mults=(8, 4, 4, 2, 1, 1),channels=3
    ).to(device)
    ...
    discriminator = discriminator_v3(
        image_size=args.hr_size, dim=32,
        dim_mults=(8, 4, 2, 2, 1),channels=3
    ).to(device)
